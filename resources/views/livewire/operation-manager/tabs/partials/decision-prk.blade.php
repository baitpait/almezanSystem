@php
    ...
    if ($eyeSuffix === 'od') {
        $epithelialRemovalField = 'recommendationForm.prk_epithelial_removal_od';
        $excimerProfileField = 'recommendationForm.prk_excimer_profile_od';
        $monovisionEyeField = 'recommendationForm.prk_monovision_eye_od';
        $targetField = 'recommendationForm.prk_target_od';
    } elseif ($eyeSuffix === 'os') {
        $epithelialRemovalField = 'recommendationForm.prk_epithelial_removal_os';
        $excimerProfileField = 'recommendationForm.prk_excimer_profile_os';
        $monovisionEyeField = 'recommendationForm.prk_monovision_eye_os';
        $targetField = 'recommendationForm.prk_target_os';
    } else {
        // Fallback to old shared fields for backward compatibility
        $epithelialRemovalField = 'recommendationForm.prk_epithelial_removal';
        $excimerProfileField = 'recommendationForm.prk_excimer_profile';
        $monovisionEyeField = 'recommendationForm.prk_monovision_eye';
        $targetField = 'recommendationForm.prk_target';
    }
@endphp

<div class="space-y-3 bg-white rounded-lg p-4 border border-gray-200" wire:key="decision-prk-{{ $eye ?? 'general' }}">
    <h4 class="font-semibold text-sm text-gray-800">PRK Details @if($eye) ({{ strtoupper($eye) }}) @endif</h4>
